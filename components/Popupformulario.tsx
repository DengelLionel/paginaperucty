...
const Popupformulario = () => {
  ...
  const generarMensajeWhatsApp = () => {
    const mensaje = `*Consulta Gratis*\n\n` +
                    `*Nombre:* ${inputNombre}\n` +
                    `*Correo:* ${inputCorreo}\n` +
                    `*Número de WhatsApp:* ${inputCelular}\n\n` +
                    `*Tipo de Negocio:* ${selectedValue}\n` +
                    `*Objetivo Principal:* ${selectedObjetivoPrincipal}\n` +
                    `*¿Tienes un diseño en mente?:* ${selectedDisenomente}\n` +
                    `*Estilo de diseño preferido:* ${selectedDisenoprefieres}\n\n` +
                    `*Comentarios Adicionales:* ${textareaValue || 'N/A'}`;
    return encodeURIComponent(mensaje);
  };

  const handleClick = () => {
    const mensaje = generarMensajeWhatsApp();
    const numeroWhatsApp = "51906064049"; // Cambia a tu número de WhatsApp
